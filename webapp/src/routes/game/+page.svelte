<script lang="ts">
    ...
    import Raining5 from '$lib/components/animations/Raining/Raining5.svelte';
    ...
    function handleCriteriaClick(index: number) {
        if (pointsCriteria[index].used === false && pointsCriteria[index].fulfilled === true) {
            ...
            // Check if it's the last criteria and show Raining5
            if (index === pointsCriteria.length - 1) {
                visible = true;
...
<main class="z-50">
    <div class="container -mt-14">
        ...
        <div class="gameBoard">
            ...
            {:else}
            {#if isGameStarting}
            <div class="countdown absolute w-full h-96 z-50 flex justify-center items-center flex-col" style="background-color: var(--primaryBg);">
                ...
                <p class="mt-20 italic font-bold py-1 px-5 rounded-full" style="background-color: var(--ghostyBg);color: var(--ghostyText);">Dieses Spiel ist frei erfunden. Aktuell werden keine Statistiken gespeichert.</p>
            </div>
            {:else}
            <header>
