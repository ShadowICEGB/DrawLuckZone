...
<main>
    <div class="container">
        ...
        <div class="board">
            <div class="top3 flex items-center justify-center">
                {#each testArray as list, i}
                {#if i < 3}
                {#if i === 1}
                <div class="relative item w-52 mx-5 mb-20 flex flex-col items-center">
                    <div class="img relative w-28 h-28" style="border-radius: 50%;background-color: var(--warning);">
                        ...
                        <img src="{list.img}" alt="UserPB" class="w-full h-full object-cover" style="border-radius: 50%;">
                        <img src="{pokal}" alt="pokal" class="w-14 absolute -top-4 -right-6">
                    </div>
                    <h1 class="uppercase font-bold text-center mt-2">
                        {truncate(list.name, 9) || 'undefined'}<br>
                        {list.highsore || 'xx.xxx.xxx'}
                    </h1>
                </div>
                {:else}
            ...
            <div class="rest mt-20 flex flex-wrap items-center justify-center">
                {#each testArray as list, i}
                {#if i > 2}
                <div class="item my-5 mx-3 w-96 rounded-full flex items-center">
                    ...
                    <div class="imgBx z-50 w-18 h-14 ml-7" style="border-radius: 50%;">
                        <img src="{list.img}" alt="UserPB" class="w-full h-full object-cover" style="border-radius: 50%;">
                    </div>
                    <h1 class="font-bold w-full -ml-5 py-2 px-1 flex pl-12 items-center rounded-full" style="background-color: var(--ghostyBg);">
                        ...
                        <span style="color: var(--ghostyText);" class="uppercase font-bold ml-2">{list.highsore || 'xx.xxx.xxx'}</span>
                    </h1>
                </div>
                {/if}
                {/each}
